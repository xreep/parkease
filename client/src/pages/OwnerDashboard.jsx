...
const defaultSchedule = () =>
  Array.from({ length: 7 }, (_, i) => ({ dayOfWeek: i, openingTime: '06:00', closingTime: '22:00', isAvailable: true }))
...
const SkeletonStat = () => (
  <div className="bg-white border border-gray-200 rounded-2xl p-5 animate-pulse">
    ...
    <div className="h-3 w-24 bg-gray-100 rounded mb-0.5"></div>
...
const OwnerDashboard = () => {
  ...
  useEffect(() => { document.title = 'Owner Dashboard - ParkEase' }, [])
  ...
  const handleApprove = async (id) => {
    ...
    try { await API.put(`/bookings/${id}/approve`); setBookings(bookings.map(b => b.id === id ? { ...b, status: 'CONFIRMED' } : b)) }
    catch { alert('Failed.') } finally { setActionLoading(null) }
  }

  const handleReject = async (id) => {
    setActionLoading(`reject-${id}`)
    try { await API.put(`/bookings/${id}/cancel`); setBookings(bookings.map(b => b.id === id ? { ...b, status: 'CANCELLED' } : b)) }
    catch { alert('Failed.') } finally { setActionLoading(null) }
  }
  ...
  const handleBlockSubmit = async (e) => {
    ...
    try {
      const { data } = await API.post('/owner/block-slot', {
        ...
        startTime: startDT.toISOString(), endTime: endDT.toISOString(), reason: blockForm.reason,
      })
    ...
    } catch (err) { setBlockError(err.response?.data?.message || 'Failed to block slot.') }
    finally { setBlockLoading(false) }
  }

  const handleDeleteSlot = async (id) => {
    if (!window.confirm('Remove this blocked slot?')) return
    try { await API.delete(`/owner/blocked-slots/${id}`); setBlockedSlots(prev => prev.filter(s => s.id !== id)) }
    catch { alert('Failed.') }
  }

  const openScheduleEdit = (listing) => {
    if (editScheduleId === listing.id) { setEditScheduleId(null); return }
    const existing = Array.isArray(listing.availabilitySchedule) && listing.availabilitySchedule.length === 7
      ? listing.availabilitySchedule : defaultSchedule()
  ...
  const updateScheduleDay = (dayOfWeek, field, value) => {
    setScheduleForm(prev => prev.map(d => d.dayOfWeek === dayOfWeek ? { ...d, [field]: value } : d))
  }

  const handleScheduleSave = async (listingId) => {
    setScheduleSaving(true); setScheduleError(''); setScheduleSuccess('')
    try {
      await API.put(`/parkings/${listingId}`, { availabilitySchedule: scheduleForm })
      setListings(prev => prev.map(l => l.id === listingId ? { ...l, availabilitySchedule: scheduleForm } : l))
      setScheduleSuccess('Schedule saved.')
      setTimeout(() => { setEditScheduleId(null); setScheduleSuccess('') }, 1500)
    } catch (err) { setScheduleError(err.response?.data?.message || 'Failed to save schedule.') }
    ...
    <div className="min-h-screen" style={{ background: '#f8fafc' }}>
      ...
      <div className="border-b border-gray-100" style={{ background: 'linear-gradient(135deg, #f8fafc 0%, #eff6ff 100%)', backgroundImage: 'radial-gradient(#2563eb12 1px, transparent 1px)', backgroundSize: '20px 20px' }}>
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-7 flex items-center justify-between">
          ...
          <Link to="/owner/add-parking" className="btn-primary text-sm text-white px-4 py-2.5 rounded-xl font-medium">Add listing</Link>
        </div>
      </div>
          ...
          <div className="bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden">
            ...
            {listings.length === 0 ? (
              <div className="text-center py-16 px-6">
                ...
                <p className="text-gray-900 font-semibold mb-1">You have no listings yet</p>
                <p className="text-gray-400 text-sm mb-5">Create your first listing to start earning.</p>
                    ...
                    <div key={l.id} className="border-b border-gray-100 last:border-b-0">
                      <div className="flex items-center hover:bg-gray-50/80 transition-colors duration-150">
                        ...
                        <div className="px-4 py-4 flex items-center gap-3">
                          <button onClick={() => openScheduleEdit(l)}
                            className="text-xs font-medium px-2.5 py-1.5 rounded-xl border transition-all duration-200 whitespace-nowrap"
                            style={isEditing ? { background: '#eff6ff', color: '#2563eb', borderColor: '#bfdbfe' } : { background: '#f8fafc', color: '#6b7280', borderColor: '#e5e7eb' }}>
                      ...
                      {isEditing && (
                        <div className="border-t border-blue-100 bg-blue-50/40 px-6 py-5">
                          ...
                          {scheduleError && <div className="bg-red-50 border border-red-200 text-red-700 rounded-xl px-3 py-2 text-xs mb-3">{scheduleError}</div>}
                          {scheduleSuccess && <div className="bg-green-50 border border-green-200 text-green-700 rounded-xl px-3 py-2 text-xs mb-3">{scheduleSuccess}</div>}
                          <div className="space-y-2 mb-4">
                            {scheduleForm.map(day => (
                              <div key={day.dayOfWeek} className="flex items-center gap-3">
                                <div className="w-8 text-center"><span className="text-xs font-semibold text-gray-500">{DAY_SHORT[day.dayOfWeek]}</span></div>
                                <button type="button" onClick={() => updateScheduleDay(day.dayOfWeek, 'isAvailable', !day.isAvailable)}
                                ...
                                {day.isAvailable ? (
                                  <div className="flex items-center gap-2 flex-1">
                                    <input type="time" value={day.openingTime} onChange={e => updateScheduleDay(day.dayOfWeek, 'openingTime', e.target.value)} className={inpSm} />
                                    <span className="text-gray-400 text-xs">to</span>
                                    <input type="time" value={day.closingTime} onChange={e => updateScheduleDay(day.dayOfWeek, 'closingTime', e.target.value)} className={inpSm} />
                                  </div>
                          ...
                          <div className="flex gap-2">
                            <button onClick={() => handleScheduleSave(l.id)} disabled={scheduleSaving}
                    ...
                    {bookings.map(b => {
                      ...
                      return (
                        <tr key={b.id} className="hover:bg-gray-50/80 transition-colors duration-150">
                          ...
                          <td className="px-6 py-4 font-semibold text-gray-900 whitespace-nowrap">{b.totalAmount ? `Rs. ${parseFloat(b.totalAmount).toFixed(0)}` : '-'}</td>
          ...
          <div className="grid lg:grid-cols-2 gap-6">
            <div className="bg-white border border-gray-200 rounded-2xl p-5 shadow-sm">
              ...
              <form onSubmit={handleBlockSubmit} className="space-y-3">
                ...
                <button type="submit" disabled={blockLoading} className="btn-primary w-full text-white text-sm font-medium py-2.5 rounded-xl disabled:opacity-60">

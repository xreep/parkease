...
const fmtRange = (start, end) => {
  if (!start || !end) return '-'
  const sd = fmtDate(start), ed = fmtDate(end)
  if (sd === ed) return `${sd}, ${fmtTime(start)} - ${fmtTime(end)}`
  return `${sd}, ${fmtTime(start)} - ${ed}, ${fmtTime(end)}`
}
...
const UserDashboard = () => {
  ...
  useEffect(() => { document.title = 'My Bookings - ParkEase' }, [])
  ...
  const handleCancel = async (id) => {
    ...
    } catch { alert('Failed to cancel booking.') } finally { setCancelling(null) }
  }
  ...
  const stats = [
    { label: 'Total bookings', value: bookings.length, borderColor: '#2563eb' },
    { label: 'Confirmed',      value: bookings.filter(b => b.status === 'CONFIRMED').length, borderColor: '#16a34a' },
    { label: 'Pending',        value: bookings.filter(b => b.status === 'PENDING').length,   borderColor: '#d97706' },
    { label: 'Completed',      value: bookings.filter(b => b.status === 'COMPLETED').length, borderColor: '#6b7280' },
  ]
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-7">
          {loading
            ...
            : stats.map(s => (
                <div key={s.label} className="bg-white border border-gray-200 rounded-2xl p-5 shadow-sm card-hover" style={{ borderTop: `3px solid ${s.borderColor}` }}>
        ...
        {disputeFor && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4 backdrop-blur-sm">
            <div className="bg-white border border-gray-200 rounded-2xl p-6 w-full max-w-md shadow-2xl animate-fadeInUp">
              ...
              {disputeError && (
                <div className="bg-red-50 border border-red-200 text-red-700 rounded-xl px-3 py-2 text-sm mb-3">{disputeError}</div>
              )}
              <form onSubmit={handleDisputeSubmit} className="space-y-3">
                <textarea
                  value={disputeMsg} onChange={e => setDisputeMsg(e.target.value)} rows={4}
                  placeholder="Describe the issue..."
                  className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none input-modern"
                  required
                />
                <div className="flex gap-2">
                  <button type="button" onClick={() => setDisputeFor(null)}
        ...
        <div className="bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden">
          ...
          {!loading && bookings.length === 0 && !error && (
            <div className="text-center py-16 px-6">
              ...
              <p className="text-gray-900 font-semibold mb-1">You have no bookings yet</p>
              <p className="text-gray-400 text-sm mb-5">Find a parking spot and make your first booking.</p>
              <Link to="/search" className="btn-primary inline-block text-sm text-white px-5 py-2.5 rounded-xl font-medium">
                Find Parking
              </Link>
            </div>
          )}
          ...
          {!loading && bookings.length > 0 && (
            <div className="divide-y divide-gray-50">
              {bookings.map(b => {
                ...
                return (
                  <div key={b.id} className="px-6 py-4 transition-colors duration-150 hover:bg-gray-50/80">
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-gray-600 mb-1">{fmtRange(b.startTime, b.endTime)}</p>
                        <div className="flex items-center gap-3 text-xs text-gray-400">
                          {duration && <span>{duration} hrs</span>}
                          {b.totalAmount != null && (
                            <span className="font-bold text-gray-900">Rs. {parseFloat(b.totalAmount).toFixed(0)}</span>
                          )}
                        </div>
                      </div>

...
const defaultSchedule = () =>
  Array.from({ length: 7 }, (_, i) => ({
    dayOfWeek: i, openingTime: '06:00', closingTime: '22:00', isAvailable: true,
  }))
...
const AddParking = () => {
  ...
  const [form, setForm] = useState({
    ...
    autoApproveBookings: true, slotSize: 'MEDIUM',
  ...
  useEffect(() => { document.title = 'List Your Space - ParkEase' }, [])
    ...
    try {
      ...
      if (photoFiles.length > 0) {
        try {
          ...
          const { data: uploadData } = await API.post('/parkings/upload-photos', fd, { headers: { 'Content-Type': 'multipart/form-data' } })
          if (Array.isArray(uploadData.urls)) photoUrls = uploadData.urls
          if (uploadData.message && uploadData.urls?.length === 0) setPhotoNotice(uploadData.message)
        } catch { setPhotoNotice('Photo upload failed — listing will be saved without photos.') }
      }
      await API.post('/parkings', {
        title: form.title, description: form.description, address: form.address, city: form.city,
        latitude: parseFloat(form.latitude) || 0, longitude: parseFloat(form.longitude) || 0,
        totalSlots: parseInt(form.totalSlots) || 1,
        vehicleTypes: form.vehicleTypes, supportedVehicleTypes: form.vehicleTypes,
        ...
        autoApproveBookings: form.autoApproveBookings, slotSize: form.slotSize,
        availabilitySchedule: form.availabilitySchedule, photos: photoUrls,
      })
      setSuccess(true)
    ...
    <div className="min-h-screen" style={{ background: '#f8fafc' }}>
      ...
      <div className="max-w-md mx-auto px-4 py-20 text-center">
        <div className="bg-white border border-gray-200 rounded-2xl p-10 shadow-sm animate-fadeInUp">
          ...
          <Link to="/owner/dashboard" className="btn-primary inline-block text-white font-semibold px-5 py-2.5 rounded-xl text-sm">
            Go to Dashboard
          </Link>
        </div>
      </div>
    ...
    <div className="min-h-screen" style={{ background: '#f8fafc' }}>
      ...
      <div className="border-b border-gray-100" style={{ background: 'linear-gradient(135deg, #f8fafc 0%, #eff6ff 100%)', backgroundImage: 'radial-gradient(#2563eb12 1px, transparent 1px)', backgroundSize: '20px 20px' }}>
        <div className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <Link to="/owner/dashboard" className="text-sm text-gray-400 hover:text-blue-600 transition-colors">&larr; Back to dashboard</Link>
      ...
      <div className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {photoNotice && <div className="bg-amber-50 border border-amber-200 text-amber-700 rounded-xl px-4 py-3 mb-5 text-sm">{photoNotice}</div>}
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <Card title="Photos">
            <p className="text-xs text-gray-400 -mt-2">Upload up to 6 photos.</p>
              ...
              <div className="space-y-2">
                {VEHICLE_OPTIONS.map(opt => (
                  <label key={opt.value}
                    ...
                    style={{ borderColor: form.vehicleTypes.includes(opt.value) ? '#2563eb' : '#e2e8f0', background: form.vehicleTypes.includes(opt.value) ? '#eff6ff' : '#fff', boxShadow: form.vehicleTypes.includes(opt.value) ? '0 0 0 3px rgba(37,99,235,0.08)' : 'none' }}>
                    <input type="checkbox" checked={form.vehicleTypes.includes(opt.value)} onChange={() => toggleVehicle(opt.value)} className="w-4 h-4 accent-blue-600" />
                    <span className="text-sm font-medium" style={{ color: form.vehicleTypes.includes(opt.value) ? '#1d4ed8' : '#374151' }}>{opt.label}</span>
                  </label>
                ))}
              </div>
            </div>
              ...
              <div className="grid grid-cols-2 gap-2">
                {SLOT_SIZES.map(s => (
                  <label key={s.value}
                    ...
                    style={{ borderColor: form.slotSize === s.value ? '#2563eb' : '#e2e8f0', background: form.slotSize === s.value ? '#eff6ff' : '#fff', boxShadow: form.slotSize === s.value ? '0 0 0 3px rgba(37,99,235,0.08)' : 'none' }}>
                    <input type="radio" name="slotSize" value={s.value} checked={form.slotSize === s.value} onChange={handleChange} className="mt-0.5 accent-blue-600" />
                    <div>
          ...
          <Card title="Pricing (in Rs.)">
            <div className="grid sm:grid-cols-3 gap-3">
              {[
                { name: 'hourlyPrice', label: 'Hourly rate', required: true, placeholder: '50' },
                { name: 'dailyPrice', label: 'Daily rate', required: false, placeholder: '400' },
                { name: 'monthlyPrice', label: 'Monthly rate', required: false, placeholder: '5000' },
              ].map(f => (
                <Field key={f.name} label={f.label} required={f.required}>
                  <div className="relative">
                    ...
                    <input type="number" name={f.name} required={f.required} min="0" step="0.01"
          ...
          <Card title="Availability schedule">
            <p className="text-xs text-gray-400 -mt-2">Set hours for each day of the week.</p>
            <div className="space-y-2">
                ...
                <div key={day.dayOfWeek} className="flex items-center gap-3">
                  ...
                  {day.isAvailable ? (
                    <div className="flex items-center gap-2 flex-1">
                      <input type="time" value={day.openingTime}
                      ...
                      <input type="time" value={day.closingTime}
          ...
          <Card title="Settings">
            <label className="flex items-center gap-3 cursor-pointer">
              <div className="w-10 h-5 rounded-full relative transition-colors duration-200 flex-shrink-0"
